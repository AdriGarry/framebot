...
function setParty(test){
	...
	setInterval(function(test){
		...
		if((hour == 23 && min == 0 || test == 3) && sec < 16) {
			var deploy = spawn('sh', ['/home/pi/odi/core/sh/soundsParty.sh', '23h']);
			console.log('23h...');
		} else if((hour == 23 && min == 30 || test == 4) && sec < 16) {
			var deploy = spawn('sh', ['/home/pi/odi/core/sh/soundsParty.sh', '23h30']);
			console.log('23h30...');
		} else if((hour == 23 && min == 59 || test == 5) && sec < 16) {
			var deploy = spawn('sh', ['/home/pi/odi/core/sh/soundsParty.sh', 'compteARebours']);
			console.log('Compte a rebours setup');
			setTimeout(function(){
				var deploy = spawn('sh', ['/home/pi/odi/core/sh/soundsParty.sh', 'compteARebours2']);
				console.log('Compte a rebours start');
			}, 2000);
		} else if((hour == 0 && min == 13|| test == 6) && sec < 16) {
			var deploy = spawn('sh', ['/home/pi/odi/core/sh/soundsParty.sh']);
			console.log('De Par mes circuits et transistors !');
			setTimeout(function(){
				var deploy = spawn('sh', ['/home/pi/odi/core/sh/soundsParty.sh', 'puisJeExprimer']);
				console.log('Puis-je m\'exprimer ?');
			}, 2000);
			setTimeout(function(){
				var deploy = spawn('sh', ['/home/pi/odi/core/sh/soundsParty.sh', 'discours']);
