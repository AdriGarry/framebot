...
app.constant('CONSTANTS', {
  ...
  DATE_REGEX: new RegExp('\\d{2}/\\d{2}[ _]', 'g'),
  MILLISEC_REGEX: new RegExp(',\\d{3} \\|', 'g'),
  LOG_LEVEL_REGEX: new RegExp('\\| [a-zA-Z]+ *\\|', 'g'),
  FILE_REGEX: new RegExp('\\| *[a-zA-Z]+.?(j|js|J|JS)?:\\d+ \\|', 'g'),
  IP_REGEX: new RegExp('\\[((?=.*\\d)(?=.*\\.)(?=.*\\:).*)_', 'g'),
  ...
  IP_LOCATION_SERVICE_URL: 'https://www.ip-tracker.org/lookup.php?ip=',
...
app.config(function ($mdThemingProvider) {
  $mdThemingProvider.theme('default').primaryPalette('teal').warnPalette('red');
});
...
app.filter('formatLog', function (CONSTANTS) {
  ...
  return function (logLine, fullLog) {
    ...
    logLine = logLine.replace(CONSTANTS.IP_REGEX, function (match, ip) {
      return '[<a href="' + CONSTANTS.IP_LOCATION_SERVICE_URL + ip + '" title="Localize IP" target="_blank">' + ip + '</a>_';
    });
    logLine = logLine.replace(CONSTANTS.GEOLOCATION_REGEX, function (match, lat, lng) {
      return '<a href="' + buildGeolocationUrlService(lat, lng) + '" title="Geolocation" target="_blank">' + match.slice(0, -1) + '</a>]';
    });
    return logLine;
  };
});
