			...
			switch (element) {
				case 'mode':
					/*if(this.value.switch){
						html = '<i class="modeSwitch far fa-rotate-270 fa-2x fa-toggle-on"></i>';
					}else{
						html = '<i class="modeSwitch far fa-rotate-270 fa-2x fa-toggle-off"></i>';
					}*/
					if (this.value.mode == 'Sleep') {
						html += '<i class="mainInfo fas fa-moon"></i>';
						if (this.value.param < 255) html += '&nbsp;' + this.value.param;
					} else {
						html +=
							'<span class="mode">' +
							this.value.mode +
							'<small><span class="desktopOnlyInline"></span>' +
							this.value.param +
							'</small></span>';
					}
					break;
				case 'switch':
					if (this.value) {
						html =
							'<md-switch class="desktopZoom switch md-primary ng-valid md-checked ng-not-empty ng-dirty ng-valid-parse ng-touched" md-no-ink="" aria-label="Switch No Ink" tabindex="0" type="checkbox" role="checkbox" aria-checked="true" aria-invalid="false"><div class="md-container" style="touch-action: pan-x;"><div class="md-bar"></div><div class="md-thumb-container"><div class="md-thumb" md-ink-ripple="" md-ink-ripple-checkbox=""></div></div></div></md-switch>';
					} else {
						html =
							'<md-switch class="desktopZoom switch md-primary ng-valid ng-dirty ng-valid-parse ng-touched ng-empty" md-no-ink="" aria-label="Switch No Ink" tabindex="0" type="checkbox" role="checkbox" aria-checked="false" aria-invalid="false"><div class="md-container" style="touch-action: pan-x;"><div class="md-bar"></div><div class="md-thumb-container"><div class="md-thumb" md-ink-ripple="" md-ink-ripple-checkbox=""></div></div></div></md-switch>';
					}
					break;
				case 'volume':
					html =
						'<i class="mainInfo fas fa-' +
						(this.value == 'high' ? 'volume-up' : this.value == 'mute' ? 'bell-slash' : 'volume-down') +
						'"></i>';
					break;
				case 'voicemail':
					html =
						...
						this.value +
						'</span>&nbsp;<i class="fas fa-' +
						(this.value > 0 ? 'envelope' : 'envelope-o') +
						'"></i>';
					break;
				case 'jukebox':
					html =
						'<i class="mainInfo fas fa-' +
						(this.value == 'jukebox' ? 'random' : this.value == 'fip' ? 'globe' : 'music') +
						'"></i>';
					break;
				case 'timer':
					html = '<i class="mainInfo fas fa-hourglass-1"></i>';
					if (this.value > 0)
						html +=
							'&nbsp;' +
							Math.trunc(this.value / 60) +
							':' +
							(this.value % 60 < 10 ? '0' + this.value % 60 : this.value % 60);
					break;
				case 'cpu':
					html = '<table class="cpu"><tr><td><i class="mainInfo fas fa-microchip"></i>&nbsp;';
					html += '<div class="value inline">' + this.value.usage + '<small>%</small></div></td>';
					html +=
						'<td><i class="mainInfo fas fa-thermometer-2"></i>&nbsp;<div class="value inline">' +
						this.value.temp +
						'<small>°C</small></div></td></tr></table>';
					break;
				case 'alarms':
					html = '<table class="alarms"><tr><td><i class="mainInfo fas fa-bell"></i></td><td><table>';
					var dayList = this.value,
						d = new Date().getDay();
					Object.keys(dayList).forEach(function(key, index) {
